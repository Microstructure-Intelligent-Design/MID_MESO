...
###########################################################*/
	...
	****************************************************************************/
		...
		static pf::Info_Phase Get_Phase_Structure(PHASES phase_property) {
			pf::Info_Phase phase;
			pf::Matrix6x6 matrix;
			double Gm = 200e8, v = 0.3, Az = 1.0, C12 = 2 * v * Gm / (1 - 2 * v), C11 = 2 * Gm / Az + C12;
			matrix(0, 0) = C11;
			matrix(1, 1) = C11;
			matrix(2, 2) = C11;
			matrix(3, 3) = Gm;
		...
		static pf::vStrain EffectivePhaseEigenStrains(pf::PhaseNode& n, pf::Info_DynamicCollection& inf) {
			...
			for (auto p = n.begin(); p < n.end(); p++) {
				double c = p->x[Cu].value;
				if (c < 0)
					c = 0;
				else if (c > 1)
					c = 1;
				// Inaccurate values, for demonstration only!
				eta0 += 0.01 * c * c * c * (10.0 - 15.0 * c + 6.0 * c * c) * p->phaseFraction;
			}
			strain[0] = eta0;
			strain[1] = eta0;
			//strain[2] = eta0;
		...
		static pf::vStrain dEffectivePhaseEigenStrains_dxCu(pf::PhaseNode& n, pf::PhaseEntry& p, pf::Info_DynamicCollection& inf) {
			pf::vStrain strain;
			double eta0 = 0.0, c = p.x[Cu].value;
			if (c < 0)
				c = 0;
			else if (c > 1)
				c = 1;
			// Inaccurate values, for demonstration only!
			eta0 = 0.03 * c * c * (10.0 - 15.0 * c + 6.0 * c * c) + 0.01 * c * c * c * (- 15.0 + 12.0 * c);
			strain[0] = eta0;
			strain[1] = eta0;
			//strain[2] = eta0;
			return strain;
		...
		static void Energy(pf::PhaseNode& N, pf::PhaseEntry& p, pf::Info_DynamicCollection& inf) {
			...
			case FCC_A1:
				...
				p.elasEnergyDensity = Get_Phase_Structure(FCC_A1).elastic_constants * (N.mechanicalValues.Strains - N.mechanicalValues.EffectiveEigenStrain)
					* (N.mechanicalValues.Strains - N.mechanicalValues.EffectiveEigenStrain)  * 0.5;
				break;
			case ALCU_THETA:
				p.chemEnergyDensity = TDB::G_ALCU_THETA(y, T, P) / vm;
				p.elasEnergyDensity = Get_Phase_Structure(ALCU_THETA).elastic_constants * (N.mechanicalValues.Strains - N.mechanicalValues.EffectiveEigenStrain)
					* (N.mechanicalValues.Strains - N.mechanicalValues.EffectiveEigenStrain) * 0.5;
				break;
			default:
		...
		static void Potential(pf::PhaseNode& N, pf::PhaseEntry& p, pf::Info_DynamicCollection& inf) {
			...
			pf::SublatticeNode& y = p.sublattice;
			double T = N.tempValues.temperature, P = 0.0, vm = MolarVolume(N, p, inf);
			pf::vStrain dstrain = dEffectivePhaseEigenStrains_dxCu(N, p, inf);
			switch (phaseProperty)
			{
			case FCC_A1:
				p.potential[Cu].chemical_part = TDB::dG_FCC_A1_dCU(x, T, P) / vm - TDB::dG_FCC_A1_dAL(x, T, P) / vm;
				p.potential[Cu].elastic_part = - (dstrain * N.mechanicalValues.Stresses);
				break;
			case ALCU_THETA:
